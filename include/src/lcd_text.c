...
int initialize_all_gpios_lcd(void)
{
	/* first lets export all the GPIOs */
	gpio_export_lcd(GPIO_66_P8_7_RS_4);
	gpio_export_lcd(GPIO_67_P8_8_RW_5);
	gpio_export_lcd(GPIO_69_P8_9_EN_6);
	gpio_export_lcd(GPIO_68_P8_10_D4_11);
	gpio_export_lcd(GPIO_45_P8_11_D5_12);
	gpio_export_lcd(GPIO_44_P8_12_D6_13);
	gpio_export_lcd(GPIO_26_P8_14_D7_14);

    
   /*first configure the direction for LCD pins */
    gpio_configure_dir_lcd(GPIO_66_P8_7_RS_4,GPIO_DIR_OUT);
    gpio_configure_dir_lcd(GPIO_67_P8_8_RW_5,GPIO_DIR_OUT);
    gpio_configure_dir_lcd(GPIO_69_P8_9_EN_6,GPIO_DIR_OUT);
    gpio_configure_dir_lcd(GPIO_68_P8_10_D4_11,GPIO_DIR_OUT);
    gpio_configure_dir_lcd(GPIO_45_P8_11_D5_12,GPIO_DIR_OUT);
    gpio_configure_dir_lcd(GPIO_44_P8_12_D6_13,GPIO_DIR_OUT);
    gpio_configure_dir_lcd(GPIO_26_P8_14_D7_14,GPIO_DIR_OUT);
...
/* Some silly graphics :) */
void tansition_graphics_lcd(void)
{
...
int main_lcdText(int argc, char *argv[])
 {
    ...
    else
    {
    	...
    	initialize_all_gpios_lcd();

        gpio_write_value_lcd(GPIO_66_P8_7_RS_4,GPIO_LOW_VALUE);
       ...
       /*The RW pin is always tied to ground in this implementation, meaning that you are only writing to
        ...
        gpio_write_value_lcd(GPIO_67_P8_8_RW_5,GPIO_LOW_VALUE);
       
       /*• The EN pin is used to tell the LCD when data is ready*/
        gpio_write_value_lcd(GPIO_69_P8_9_EN_6,GPIO_LOW_VALUE);
        
        /*Data pins 4~7 are used for actually transmitting data, and data pins 0~3 are left unconnected*/
        gpio_write_value_lcd(GPIO_68_P8_10_D4_11,GPIO_LOW_VALUE);
        gpio_write_value_lcd(GPIO_45_P8_11_D5_12,GPIO_LOW_VALUE);
        gpio_write_value_lcd(GPIO_44_P8_12_D6_13,GPIO_LOW_VALUE);
        gpio_write_value_lcd(GPIO_26_P8_14_D7_14,GPIO_LOW_VALUE);
        ...
        while(1)
        {
             ...
             tansition_graphics_lcd();
             print_ip_address_lcd();
             tansition_graphics_lcd();
             lcd_printf(msg_buf);
             tansition_graphics_lcd();
             print_time_and_date_lcd();
             tansition_graphics_lcd();
        }
    }
