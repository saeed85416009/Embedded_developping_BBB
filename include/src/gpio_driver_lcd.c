 ...
 *
 */
int gpio_export_lcd(uint32_t gpio_num)
{
	int fd, len;
	char buf[SOME_BYTES];
 ...
 */
int gpio_configure_dir_lcd(uint32_t gpio_num, uint8_t dir_value)
{
    int fd;
    char buf[SOME_BYTES];
 ...
 */
int gpio_write_value_lcd(uint32_t gpio_num, uint8_t out_value)
{
    int fd;
    char buf[SOME_BYTES];
 ...
 *  GPIO read value
 */
int gpio_read_value_lcd(uint32_t gpio_num)
{
    int fd;
 ...
 *  edge : rising, falling
 */
int gpio_configure_edge_lcd(uint32_t gpio_num, char *edge)
{
    int fd;
    char buf[SOME_BYTES];
 ...
 */
int gpio_file_open_lcd(uint32_t gpio_num)
{
    int fd;
    char buf[SOME_BYTES];
 ...
 *  close a file
 */
int gpio_file_close_lcd(int fd)
{
    return close(fd);
}
